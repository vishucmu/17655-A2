		...
		if (em != null)
		{
			...
			*********************************************************************/
			...
			while ( !Done )
			{
				...
				{
					Msg = eq.GetMessage();
					//mw.WriteMessage( String.format("TC miss: %d, flag:%b, HC miss: %d, flag:%b", TControllerMiss, TControllerFlag, HControllerMiss, HControllerFlag));
					//mw.WriteMessage( String.format("TS miss: %d, flag:%b, HS miss: %d, flag:%b", TSensorMiss, TSensorFlag, HSensorMiss, HSensorFlag));
					if ( Msg.GetMessageId() == 1 ) // Temperature reading and check Temperature Sensor is alive
					{
					    TSensorFlag=true; 
					    TSensorMiss=0;
					...
					if ( Msg.GetMessageId() == 2 ) // Humidity reading and check Humidity Sensor is alive
					{
					    HSensorFlag=true;
					    HSensorMiss=0;
